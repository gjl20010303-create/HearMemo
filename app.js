...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function syncProgressToServer() {
        ...
        try {
            ...
            const allWords = Object.values(ebData);
            const totalSeen = allWords.length;
            const mastered  = allWords.filter(w => w.nextReviewDate === '2099-12-31').length;
            const totalMistakes = allWords.reduce((s, w) => s + (w.mistakes || 0), 0);
            const todayDate = new Date().toISOString().slice(0, 10);
            // 今日接触词数：今天 lastReviewDate 的词
            const todayWords = allWords.filter(w => w.lastReviewDate === todayDate).length;

            await fetch('/api/progress/sync', {
                method: 'POST',
                headers: authHeaders(),
                body: JSON.stringify({ totalSeen, mastered, totalMistakes, todayWords, todayDate, ebSnapshot: ebData })
            });
        } catch (e) { console.warn('Progress sync failed:', e); }
    }

    // 页面离开时静默上传
    window.addEventListener('beforeunload', () => { syncProgressToServer(); });

    function loadSprintWord() {
        if (sprintIndex >= sprintList.length) {
            _clearSprintSession();   // 本批完成，清除会话
            syncProgressToServer();  // 上传进度
    ...
    async function renderStudentProgress() {
        ...
        try {
            ...
            if (!resp.ok) { wrap.innerHTML = '<div class="empty-state">权限不足或加载失败</div>'; return; }
